...
class PopupCompany(QDialog):
    def change_avatar(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Chọn ảnh đại diện", "", "Image Files (*.png *.jpg *.jpeg *.bmp)"
        )
    ...
    def __init__(self, parent=None):
        ...
        if company and company.get("image_path"):
            company_icon = company["image_path"]
        else:
            company_icon = APP_ICON_PATH
        self.setWindowIcon(QIcon(company_icon))

        # Đảm bảo popup nằm giữa main_window nếu có parent
        if parent is not None:
            parent_geom = (
                parent.geometry()
            )  # geometry() lấy vị trí thực tế trên màn hình
        ...
        name_group = self._create_input_group(
            "Tên công ty", "Nhập tên công ty...", company.get("name", "")
        )
        ...
        address_group = self._create_input_group(
            "Địa chỉ", "Nhập địa chỉ...", company.get("address", "")
        )
        ...
        phone_group = self._create_input_group(
            "Số điện thoại", "Nhập số điện thoại...", company.get("phone", "")
        )
    ...
    def _create_input_group(
        self, label_text: str, placeholder: str, default_value: str
    ) -> dict:
    ...
    def _create_avatar_section(self, company: dict) -> QWidget:
        ...
        self.btn_change_image.setStyleSheet(
            f"background: {BUTTON_COLOR}; color: white; border-radius: 4px; font-weight: {FONT_BOLD};"
        )
